...
extension RationalNumber {
  var numer: Int { numerator }
  var denom: Int { denominator }
}
...
extension Interval {
  static func + (lhs: Interval, rhs: Interval) -> Interval {
    Interval(lhs.lower + rhs.lower, lhs.upper + rhs.upper)
  }
  ...
  static func / (lhs: Interval, rhs: Interval) -> Interval {
    lhs * Interval(1.0 / rhs.upper, 1.0 / rhs.lower)
  }
}
...
extension Array {
  ...
  func accumulate<Result>(_ initial: Result, _ combine: (Element, Result) -> Result) -> Result {
    self.reduce(initial) { result, element in
...
struct ListSet<T: Equatable> {
  ...
  func union(_ other: ListSet<T>) -> ListSet<T> {
    other.elements.reduce(self) { set, element in
      set.insert(element)
    }
  }

  func intersection(_ other: ListSet<T>) -> ListSet<T> {
    ListSet(elements.filter { other.contains($0) })
  }
}
...
struct RectangularComplex: ComplexNumber {
  ...
  var realPart: Double { real }
  var imagPart: Double { imag }
  var magnitude: Double { sqrt(real * real + imag * imag) }
  var angle: Double { atan2(imag, real) }
}
...
struct PolarComplex: ComplexNumber {
  ...
  var realPart: Double { mag * cos(ang) }
  var imagPart: Double { mag * sin(ang) }
  var magnitude: Double { mag }
  var angle: Double { ang }
}
...
extension RationalNumber: ArithmeticType {
  static var zero: RationalNumber { RationalNumber(0, 1) }
  static var one: RationalNumber { RationalNumber(1, 1) }
}
