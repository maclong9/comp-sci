...
func createGCDMachine() -> RegisterMachine {
  ...
  machine.addInstruction(
    .assign(register: "a") {
      machine.getRegister("b") ?? 0
    })
  machine.addInstruction(
    .assign(register: "b") {
      machine.getRegister("t") ?? 0
    })
...
class LispHeap {
  ...
  func car(of address: Int) -> Any? {
    consTable[address]?.car
  }

  func cdr(of address: Int) -> Any? {
    consTable[address]?.cdr
  }
...
class ExplicitControlEvaluator {
  ...
  private func isNumber(_ exp: Any) -> Bool { exp is Double || exp is Int }
  private func isString(_ exp: Any) -> Bool { exp is String }
  private func isSymbol(_ exp: Any) -> Bool { exp is String }
  private func isPrimitive(_ proc: Any) -> Bool { false }
  private func lookupVariable(_ symbol: Any, in env: Any) -> Any { "" }
  private func applyPrimitive(_ proc: Any, args: [Any]) -> Any { "" }
  private func extendEnvironment(_ proc: Any, args: [Any], baseEnv: Any) -> Any { "" }
}
