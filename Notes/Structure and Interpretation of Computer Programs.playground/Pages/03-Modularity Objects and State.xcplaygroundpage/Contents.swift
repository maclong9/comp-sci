...
class Table<Key: Hashable, Value> {
  ...
  func display() -> String {
    "Table: \(records.map { "(\($0.0): \($0.1))" }.joined(separator: ", "))"
  }
}
...
class SerializedBankAccount {
  ...
  func transfer(
    to otherAccount: SerializedBankAccount, amount: Double, completion: @escaping (String) -> Void
  ) {
...
func randomStream(seed: UInt64) -> Stream<Double> {
  ...
  return Stream(
    head: {
      currentSeed = (currentSeed &* 1_103_515_245 &+ 12345) & 0x7fff_ffff
      return Double(currentSeed) / Double(0x7fff_ffff)
    },
